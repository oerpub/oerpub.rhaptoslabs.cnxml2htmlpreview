...
MathJax.Extension.mml2jax = {
  ...
  PreProcess: function (element) {
    ...
    this.ProcessMathArray(element.getElementsByTagName("m:math"));
    this.ProcessMathArray(element.getElementsByTagName("mml:math"));
    this.ProcessMathArray(element.getElementsByTagName("math:math"));
    //
  ...
  ProcessMathFlattened: function (math) {
    ...
    while (math && math.nodeName.search(/\/(\w+:)?MATH/) == -1) {
      node = math; math = math.nextSibling;
      mml += this.NodeHTML(node);
      node.parentNode.removeChild(node);
    }
    if (math && math.nodeName.search(/\/(\w+:)?MATH/) != -1) {
      mml += this.NodeHTML(math)
      math.parentNode.removeChild(math)
    }
    script.text = mml.replace(/&/g,"&amp;").replace(/</g,"&lt;");;
    if (this.config.preview !== "none") {this.createPreview(MATH,script)}
  },
